...
In this study they sampled fish, grass, pines, plankton, and POM for bulk tissue carbon and nitrogen isotope analyses. The figure shows that there is considerable overlap and high variance in these groups, making it difficult to determine relative allochthonous and autochthonous contributions to consumers.
...
d13C.d15N<-ggplot(bulk.iso, aes(x=d13C, y=d15N, fill=Type, shape=Type)) +
  ...
  scale_shape_manual(values=c(21,22,22,24,21)) +
  scale_fill_manual(values=c("orchid", "lightgoldenrod3", "lightsalmon3", "dodgerblue", "black")) +
  stat_ellipse(aes(color=Type)) +
  scale_color_manual(values=c("orchid", "lightgoldenrod3", "lightsalmon3", "dodgerblue", "black")) +
...
dev.copy(pdf, "figures/Fig2.SNL.EssAA.Boxplot_rev.pdf", width = 10, height = 5)
dev.off()
```
...
######
pdf(file= "figures/FigS4.PCA.location_rev.pdf", height=6, width=8)
...
### export it
#pdf(file= "figures/Fig3A.PCA.norm.plank.pdf", height=6, width=8)
...
plot_grid(Source.LDA, LDA.scat.Group, LDA.scat.phygroup, ncol=3, labels=c("A", "B", "C"), rel_widths = c(1,1.2,1.2))
dev.copy(pdf, "figures/FigSx.Source.Plank.scat.LDA3.pdf", width = 14, height = 4)
dev.off()
```

Combine LD1-boxplots and PCA
```{r combined PCA and LDA taxa plot}
#combine PCA and LDA
extract.legend.PCA <- get_legend(
  # create some space to the left of the legend
  PCA.norm.plot + theme(legend.text=element_text(size=8)) + theme(legend.box.margin = margin(0, 0, 0, 10)))


Fig.3<-plot_grid(PCA.norm.plot+guides(color = FALSE, shape=FALSE), LDA.phy.boxplot, extract.legend.PCA, 
                 labels=c("A", "B", ""), ncol=3,
                 rel_widths = c(1,0.8,0.3), rel_heights = c(1.5,1,0.1))
Fig.3
dev.copy(pdf, "figures/Fig3ab.PCA.LDA.pdf", height = 5, width = 10)
dev.off()
```
