...
Fig.formatting <- (theme_classic()) +
  theme(text = element_text(size = 10),
        ...
        axis.text.y = element_text(
          margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"), color = "black", size = 10), 
        axis.text.x = element_text(
          margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"), color = "black", size = 8)) +
...
### Site Maps
# THIS CODE DOES NOT WORK FOR ALEXI 2025-05-09
...
# saved as jpeg to preserve the permill symbol
...
Read in the full data set (corrected using the NMX in-house reference material; not to be confused with mean-normalized data) and pull out just the essential amino acid (AA~ESS~) data, then subset by sample **Type**. We will be excluding Lysine due to poor peak size and/or separation.
```{r essAA load}
...
colors2 <- c("springgreen4", "dodgerblue", "lightsalmon3", "gray40")
...
EAA.box <- ggplot(EAA.df.long, aes(x = EAA, y = d13C, fill = Group))+
  ...
  #geom_jitter(aes(fill=Group), color="black",pch=21, size=1)+
  scale_fill_manual(values = colors2)+ 
  ylab(expression(paste(delta^{13}, C, " (\U2030)"))) +
  xlab(element_blank()) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 12, color = "black"),
        axis.text.y = element_text(size = 12, color = "black"),
...
EAA.box.nrm <- ggplot(EAA.df.long, aes(x = EAA, y = d13C.n, fill = Group))+
  geom_boxplot(color = "black")+
  #geom_jitter(aes(fill=Group), color="black",pch=21, size=1)+
  scale_fill_manual(values = colors2, breaks = c("Algae", "POM", "Terrestrial Plants", "Zooplankton"),
                    labels = c("Benthic Algae", "POM", "Terrestrial Plants", "Zooplankton")) + 
  ylab(expression(paste(delta^{13}, C[norm], " (\U2030)"))) +
  xlab(element_blank()) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 12, color = "black"),
        axis.text.y = element_text(size = 12, color = "black"),
...
jpeg("figures/FigS3.SNL.EssAA.Boxplot_rev.jpeg", width = 10, height = 5, units = "in", res = 300)
EAA.box.raw.nrm
dev.off()
...
indiv.EAA.plot <- ggplot(EAA.long.grouped, aes(x = EAA, y = Mean.d13C, fill = Lake)) +
  ...
  geom_line(data = EAA.long.grouped, aes(x = EAA, y = Mean.d13C, color = Lake,
                                         group = Lake), position=dodge, linewidth = 0.75) +
...
indiv.EAA.plot.norm <- ggplot(EAA.long.grouped, aes(x = EAA, y = Mean.d13C.n,
                                                    fill = Lake)) +
  facet_wrap(. ~ Group) +
  theme_bw() +
  geom_errorbar(aes(ymin = Mean.d13C.n-SE.d13C.n, ymax = Mean.d13C.n+SE.d13C.n,
                    color = Lake),
                position = dodge, width = 0.0, linewidth = 0.75) +
  geom_line(data = EAA.long.grouped, aes(x = EAA, y = Mean.d13C.n, color = Lake,
                                         group = Lake),
...
EAA.raw.nrm.lines <- plot_grid(indiv.EAA.plot, indiv.EAA.plot.norm, rel_widths = c(1.65,2.1),
          labels = c("A", "B"))
jpeg("figures/FigS4.indivAA_Lines_rev.jpeg", height = 9, width = 12, units = "in", res = 300)
EAA.raw.nrm.lines
dev.off()
...
#### Linear models: AA~ESS~ and AA~ESS-norm~ 
...
```{r linear models measured data, results='hide', fig.show='hide', collapse=TRUE}
...
print(anova(Ile.mod), digits = 4) # p=0.013
...
print(anova(Leu.mod), digits = 4) # p<0.001
...
print(anova(Phe.mod), digits = 4) # p<0.001
...
posthoc.Val <- emmeans(Val.mod, ~ Group)
multcomp::cld(posthoc.Val, Letters = letters) # POM (A), Zoop (A), Plants (A), Algae (A)
...
Using the AA~ESS-norm~ δ^13^C data, we will use a PERMANOVA to test whether the four Groups (POM, algae, terrestrial plants, zooplankton) differ in multivariate space. Considering we observed significant site-level differences due to isotopic variation at the base of the food web, using mean-normalized data is best.  

#### Permanova: **Lake** and **Group** effects 
Use Permanova to test for differences among sites in measured and normalized δ^13^C values of zooplankton.
  
*Lake effects*
- Investigate both raw and mean-normalized data.
- Remove sources, since we have a small sample size.
- Run PERMANOVA to test for Lake effects on zooplankton only. 
- This is part of Table S4.

This PERMANOVA (`EAA.raw.perm.Lake.zoop`) uses measured (non-normalized) data to test for Lake effects on zooplankton.
```{r rawEAA perman Lake}

# master data frame = EAA.df.wide
# measured (non-normalized) data frame = ESS.df.raw

# remove the sources (Algae, POM, and Plants) to just look at the zooplankton, which have better site-level replication
PCA.raw.zoop<-ESS.df.raw[!(ESS.df.raw$phy.group == "Algae" | 
                        ESS.df.raw$phy.group == "POM" | 
                        ESS.df.raw$phy.group == "Plants"),]

# factors: Lake and phy.group (cladocera, copepoda, >350um)
...
############ RAW DATA - Zooplankton only ##############
# run a PERMANOVA to test for differences among Lakes #
#######################################################

# run PERMANOVA
set.seed(213)
EAA.raw.perm.Lake.zoop <- adonis2(PCA.dat.raw.zoop ~ Lake, data = PCA.fac.zoop, permutations = 999, method = "euclidian", by = "terms")
EAA.raw.perm.Lake.zoop # zooplankton only across Lakes: different at p=0.001, R2=0.87 

pairwise.adonis(PCA.dat.raw.zoop, factors = c(PCA.fac.zoop$Lake), sim.method = "euclidian", p.adjust.m = "none")

```

This PERMANOVA (`EAA.norm.perm.Lake.zoop`) uses mean-normalized data to test for Lake effects on zooplankton.
```{r normEAA perman Lake}

# master data frame = EAA.df.wide
# normalized data frame = ESS.df.norm

# remove the sources (Algae, POM, and Plants) to just look at the zooplankton, which have better site-level replication
PCA.norm.zoop <- ESS.df.norm[!(ESS.df.norm$phy.group == "Algae" | 
                        ESS.df.norm$phy.group == "POM" | 
                        ESS.df.norm$phy.group == "Plants"),]

# factors: Lake and phy.group (cladocera, copepoda, >350um)
PCA.fac.norm.zoop <- PCA.norm.zoop %>%
  dplyr::select(Lake, phy.group)

# response variables: each EAA
PCA.dat.norm.zoop <- PCA.norm.zoop %>%
  dplyr::select(Ile.n, Leu.n, Phe.n, Thr.n, Val.n)

####### MEAN-NORMALIZED DATA - Zooplankton only #######
# run a PERMANOVA to test for differences among Lakes #
#######################################################

# run PERMANOVA
set.seed(312)
EAA.norm.perm.Lake.zoop <- adonis2(PCA.dat.norm.zoop ~ Lake, data = PCA.fac.norm.zoop, permutations = 999, method = "euclidian",by = "terms")
EAA.norm.perm.Lake.zoop # zooplankton only across Lakes: different at p=0.068, R2=0.35

# run pairwise 
set.seed(312)
pairwise.adonis(PCA.dat.raw.zoop, factors = c(PCA.fac.norm.zoop$Lake), sim.method = "euclidian", p.adjust.m = "none")
...
EAA.raw.perm.Group # Groups: different at p=0.001, R2=0.49
...
# run PERMANOVA
set.seed(51)
EAA.norm.perm.Group <- adonis2(ESS.n.dat ~ Group, data = Group.n.fac, permutations = 999, method = "euclidian", by = "terms")
...
# run PERMANOVA
set.seed(51)
EAA.perm.Source<-adonis2(ESS.norm.dat.source ~ Group, data = Source.n.fac, permutations = 999, method = "euclidian", by = "terms")
EAA.perm.Source
...
#### PCA
This PCA (`PC.raw.zoop`) uses measured (non-normalized) data to test for Lake effects on zooplankton.
```{r PCA raw, eval=FALSE}

### the PCA below is not included in the manuscript ###

# perform a PCA on the raw (non-normalized) zooplankton data to investigate differences in whole zooplankton communities across Lakes
...
PC.raw.zoop.sum
#plot(PC.raw.zoop, type = "l", main = "PC.area eigenvalues")
# 2 PCs explain 95.8% of variation, most in PC1 (93.4%)

PCA.fac.zoop$phy.group <- droplevels(PCA.fac.zoop$phy.group)

# plot PCA (PC1 vs PC2)
PCA.raw.Lake.zoop <- ggbiplot::ggbiplot(PC.raw.zoop, choices = 1:2, obs.scale = 1, var.scale = 1, 
  ...
  ggtitle(expression(paste(AA[ESS], " ", delta^{13}, C))) +
  theme_classic() + 
...
PCA.norm.sorc.plot <- ggbiplot::ggbiplot(PC.norm.sorc, choices = 1:2, obs.scale = 1, var.scale = 1, 
  ...
  theme(axis.ticks.length = unit(0.2, "cm"),
        ...
        axis.text.y = element_text(margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"), color = "black"),
        axis.text.x = element_text(margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"), color = "black")) +
  guides(shape = FALSE) + guides(fill = FALSE) + guides(color = FALSE) +
...
```{r PCA sources and zoop, eval=FALSE}
...
PC.n.fac <- ESS.norm.perm  %>% 
  dplyr::select(Lake, Group, phy.group, category)
...
## PC1 and PC2
PCA.norm.plot <- ggbiplot::ggbiplot(PC.norm, choices = 1:2, obs.scale = 1, var.scale = 1, 
                      ...
                      circle = FALSE) +
  geom_point(size = 3.5, color="black",
             aes(fill = PC.n.fac$phy.group, shape = ESS.norm.perm$category)) +
  geom_vline(xintercept = 0, linetype="dashed", color = "gray60")+
  geom_hline(yintercept = 0, linetype="dashed", color = "gray60")+
...
#### LDA by sample-site # THIS IS LEFTOVER FROM WHEN WE ONLY HAD TWO SOURCES (POM AND TERRESTRIAL PLANTS) AND THUS, ONLY ONE LD!
...
phy.lda.pts<-c(24,24,24, 21, 22,22, 23,23,23)
...
LDA.spp.Lake
dev.copy(pdf, "figures/FigS5.LDA.spp.Lake_rev.pdf", width = 8, height = 6)
dev.off()
```

#### LDA by taxa # THIS IS LEFTOVER FROM WHEN WE ONLY HAD TWO SOURCES (POM AND TERRESTRIAL PLANTS) AND THUS, ONLY ONE LD!
...
LDA.df$phy.group<-factor(LDA.df$phy.group, levels=c("Algae", "POM", ">350um", "cladocera", 
                                                            "copepoda",
                                                            "Plants"))
LDA.df$category<-ifelse(LDA.df$Group == "Zooplankton", "Zooplankton", "Producer")
LDA.df$category<-as.factor(LDA.df$category)


LDA.phy.boxplot<-ggplot(LDA.df, aes(x=LD1, y=phy.group, 
                                 fill = phy.group))+
  geom_boxplot(alpha=0.9, outlier.color=NA)+
  geom_jitter(aes(fill=phy.group, shape=category), colour="black", size=2, show.legend = F) +
  geom_vline(xintercept=1.5, linetype="dashed", color = "gray60")+
...
#LDA.phy.boxplot
#dev.copy(pdf, "figures/Fig3b.LDA.phy.boxplot.pdf", height = 6, width = 6)
#dev.off()
```
...
lda.4.colors <- c("springgreen4", "dodgerblue", "lightsalmon3", "gray30")
...
LDA.scat.phygroup <- ggplot(LDA.df, aes(x = LD1, y = LD2)) +
  ...
  scale_fill_manual(values = lda.6.colors.main, 
                    breaks = c("Algae", "POM", "Plants", ">350um", "cladocera", "copepoda"),
  ...
  scale_color_manual(values = lda.6.colors, guide = NULL) +
  scale_linetype_manual(values = c(2, 2, 0, 0, 0, 2), guide = NULL) +
  xlab("Linear Discrimant 1 (85.8%)") + ylab("Linear Discriminant 2 (14.2%)") +
  theme_classic() + 
  ...
  #annotate("text", x = 3.8, y = -4.9, label = expression(beta[Leu] == -0.721 ~ ";" ~ beta[Phe] == 0.563),
           #color = "black", size = 3) +
  #annotate("text", x = -6.2, y = 2.8, label = expression(beta[Phe] == 0.385 ~ ";" ~ beta[Val] == -0.237),
...
LDA.compiled <- plot_grid(Source.LDA, LDA.scat.Group, LDA.scat.phygroup, ncol = 3, labels = c("A", "B", "C"), rel_widths = c(1,1.2,1.2))

```


### MixSIAR
We will first run MixSIAR to determine the proportion of algae/terrestrial material in the POM. This will help us validate the use of POM as a food source and its autochthony composition. #I THINK WE CUT THIS???
```{r}
...
Mix.df.all <- LDA.df %>%
  dplyr::select("SampleID", "Group", "orig.class", "phy.group", "Lake", "Elevation.m", "LD1")

#Keep just POM as "consumer"
POM.df <- Mix.df.all[(Mix.df.all$Group=="POM"),]
POM.summary.phy.Lake<-aggregate(LD1~Lake+phy.group, data=POM.df, FUN=length)

#Write and export a csv of the zooplankton data frame
write.csv(POM.df, "output/POM.Mix.df.csv", row.names=FALSE)

#Create a new data frame with desired columns
Source.2.df <-  Mix.df.all[(Mix.df.all$phy.group=="Plants" | 
                            Mix.df.all$phy.group=="Algae"),]

#drop levels and subset
Source.2.df$phy.group<-droplevels(Source.2.df$phy.group)
Source.2.df <- Source.2.df %>%
  dplyr::select("phy.group", "LD1")

#Calculate mean, SD, and length LD1 values of the sources
mean.df.mix <- aggregate(LD1~phy.group, data=Source.2.df, FUN=mean)
sd.df.mix <- aggregate(LD1~phy.group, data=Source.2.df, FUN=sd)
n.df.mix <- aggregate(LD1~phy.group, data=Source.2.df, FUN=length)

#Bind the values calculated above into one data frame
source.2.agg.df <- cbind(n.df.mix, mean.df.mix[2], sd.df.mix[2])

#Rename the columns in this data frame
colnames(source.2.agg.df) <- c("phy.group", "n", "MeanLD1", "SDLD1")

#Write and export a csv of the source data frame
write.csv(source.2.agg.df, "output/source.2.agg.df.csv", row.names=FALSE)

###################
#Load zooplankton (consumer) data and assign factors
cons <- load_mix_data(filename = "output/POM.Mix.df.csv",
                          iso_names=c("LD1"),
                          factors=c("SampleID"),
                          fac_random=c(FALSE),
                          fac_nested=c(FALSE),
...
#Load source data
source <- load_source_data(filename="output/source.2.agg.df.csv",
                             source_factors=NULL,
                             conc_dep=FALSE,
                             data_type="means",
                             cons)
...
#Load TDF data
discr <- load_discr_data(filename="data/MixSIAR_prod.TDFs.csv", cons)

```

Export AA~ESS~ source and zooplankton LDA data to feed into MixSIAR. We will calculate the mean, standard deviations, and sample sizes for the LD1 and LD2 values of the sources.
```{r mixsiar setup}
...
Source.df <-  Mix.df.all[(Mix.df.all$orig.class == "Algae" | Mix.df.all$orig.class == "POM" |
                            Mix.df.all$orig.class == "Terrestrial Plants"),]
...
colnames(source.agg.df) <- c(NA, "n", "MeanLD1", "SDLD1", "MeanLD2", "SDLD2")

# write and export a csv of the source data frame
write.csv(source.agg.df, "output/source.agg.df.csv", row.names = FALSE, na = "")
# delete the "NA" from the first row/first column in the csv file and resave
...
#Define the model and error structure and write JAGS model file
  model_filename <- "output/MixSIAR_models/SampleID_prod_models/MixSIAR_Model_SampleID_fixed.txt"
  resid_err <- FALSE
  process_err <- TRUE
...
#Process diagnostics, summary stats, and posterior plots
  output_JAGS(invt.jags.mod, cons, source, output_options=list(
    summary_save = TRUE,                   
    summary_name = "output/MixSIAR_models/SampleID_prod_models/MixSIAR_SampleID_summary_stats",
    sup_post = FALSE,                       
    plot_post_save_pdf = TRUE,              
    plot_post_name = "output/MixSIAR_models/SampleID_prod_models/MixSIAR_SampleID_posterior_density",   
    sup_pairs = TRUE,                       
    plot_pairs_save_pdf = TRUE,             
    plot_pairs_name = "output/MixSIAR_models/SampleID_prod_models/MixSIAR_SampleID_pairs_plot",         
    sup_xy = TRUE,                          
    plot_xy_save_pdf = TRUE,                
    plot_xy_name = "output/MixSIAR_models/SampleID_prod_models/MixSIAR_SampleID_xy_plot",             
    gelman = TRUE,                         
    heidel = FALSE,                         
    geweke = TRUE,                         
    diag_save = TRUE,                       
    diag_name = "output/MixSIAR_models/SampleID_prod_models/MixSIAR_SampleID_diagnostics",
    indiv_effect = FALSE,
...
#save all results
save.image("output/MixSIAR_models/SampleID_prod_models/MixSIAR_SampleID_fixed_data.RData")

#############################
SAMP.mixSIAR.output<-read.table("output/MixSIAR_models/SampleID_prod_models/MixSIAR_SampleID_summary_stats.txt", sep='\t', header=TRUE, skip=6)
...
write.csv(Samp.out.cleaned, "output/MixSIAR_prod_Samp.df.csv", row.names=FALSE)
############
aggregate(Mean~Source, data=Samp.out.cleaned, FUN=mean)
aggregate(Mean~Source, data=Samp.out.cleaned, FUN=sd)

#plot it
Samp.out.cleaned$Group<-as.factor(Samp.out.cleaned$Source)
Samp.out.cleaned$Mean<-as.numeric(Samp.out.cleaned$Mean)

POM.mixsiar.plot<-ggplot(Samp.out.cleaned, aes(y=Mean, x=Source))+
geom_boxplot(alpha=0.8, aes(fill=Source))+
  geom_jitter(aes(fill=Source), colour="black",pch=21, size=2)+
  scale_fill_manual(values=colors2)+ 
  coord_cartesian(ylim=c(0, 1)) +
  ylab(expression(paste("Energy channel proportion"))) +
  xlab("Source") +
  theme_classic()
...
geom_boxplot(alpha=0.8, aes(fill=Source))+
  geom_jitter(aes(fill=Source), colour="black",pch=21, size=2)+
  scale_fill_manual(values =c("springgreen4", "lightsalmon3")) +
  scale_color_manual(values =c("springgreen4", "lightsalmon3")) +
  coord_cartesian(ylim=c(0, 1)) +
  ylab(expression(paste("Energy channel proportion"))) +
  xlab("Source") +
  theme_classic()

pdf(file= "figures/Figx.Phy.mixsiar.pdf", height=4, width=3)
POM.mixsiar.plot
dev.off()
```
...
isospace_plot <- plot_data(filename = "output/MixSIAR_models/SampleID_models/MixSIAR_isospace_plot_SampleIDs",
                           plot_save_pdf = TRUE, plot_save_png = FALSE,
                           cons, source, discr)
...
# process diagnostics, summary stats, and posterior plots
  output_JAGS(zoop.jags.mod, cons, source, output_options = list(
    summary_save = TRUE,                   
...
zoop.auto.combined <- combine_sources(zoop.jags.mod, cons, source,
...
print(summary_stat(zoop.auto.combined))
sink()

```

```{r mixsiar by sample results, eval=FALSE, echo=TRUE, fig.keep='none'}
...
SAMP.mixSIAR.1.output <- read.table("output/MixSIAR_models/SampleID_models/MixSIAR_SampleID_summary_stats_a_posteriori.txt",
...
# rename columns
library(plyr)
samp.mix.1.out<-separate(SAMP.mixSIAR.1.output, full.data, into = c("factor", "Mean", "SD", "2.5.perc", "5.perc", "25.perc", "50.perc", "75.perc", "95.perc", "97.5.perc"), sep=" ")
...
MSiar.LD.meta.EAA<- MSiar.LD.meta.EAA %>%
  dplyr::select("SampleID", "Source", "Mean", "SD", "Lake.x",
                "orig.class.x", "phy.group.x", "LD.class", "LD1", "Group.x", 
                "Ile", "Leu", "Phe", "Thr", "Val")

MSiar.LD.meta.EAA<- MSiar.LD.meta.EAA %>% 
  dplyr::rename("Lake" = "Lake.x", "orig.class" = "orig.class.x", "phy.group" = "phy.group.x", "Group" = "Group.x")
...
MSiar.data.env<- MSiar.data.env %>%
  dplyr::select("Location", "Date", "Lake", "elevation..m", "latitude", "longitude", "depth..m",
                ...
                "TDN..ug.L", "TDP..ug.L", "chla..ug.L", "SampleID", "Group", "orig.class", "phy.group", 
                "Ile", "Leu", "Phe", "Thr", "Val",
...
# library(MASS)
# run a full model
mod.mult<-lm(prop.plant.mean~temp..C+ pH+TDN..ug.L+chla..ug.L,
             data=Plant.nut.env)
...
mod.mult.all<-lm(prop.plant.mean~TDN..ug.L+chla..ug.L,
             data=Plant.nut.env)
...
# TDN, all Groups
N.prop.plant<-ggplot(Plant.nut.env, aes(x=TDN..ug.L, y=prop.plant.mean)) +
...
mod.mult.cladoc<-lm(prop.plant.mean~temp..C+ pH+TDN..ug.L+chla..ug.L,
             data=cladoc.plant)
...
mod.mult<-lm(prop.plant.mean~TDN..ug.L+chla..ug.L,
             data=cladoc.plant)
summary(mod.mult)
...
##### Copepods
# run a full model, but 3 not defined because of singularities
mod.mult.copepod<-lm(prop.plant.mean~temp..C+pH+TDN..ug.L+chla..ug.L,
             data=copep.plant)
...
mod.mult.copepod<-lm(prop.plant.mean~TDN..ug.L+temp..C,
             data=copep.plant)
summary(mod.mult.copepod)
...
copepod.TN<-ggplot(copep.plant, aes(x=TDN..ug.L, y=prop.plant.mean)) +
