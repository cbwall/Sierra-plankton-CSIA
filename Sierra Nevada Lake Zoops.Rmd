...
print(anova(Ile.mod), digits=6) # p=0.013
plot(allEffects(Ile.mod), ylab="Ile", par.strip.text=list(cex=0.7))

posthoc.Ile<-emmeans(Ile.mod, ~Group)
multcomp::cld(posthoc.Ile, Letters=letters) # POM (A), Zoop (A), Plant (AB), Algae (B)
...
print(anova(Thr.mod), digits=6) # p<0.001
plot(allEffects(Thr.mod), ylab="Thr", par.strip.text=list(cex=0.7))

posthoc.Thr<-emmeans(Thr.mod, ~Group)
multcomp::cld(posthoc.Thr, Letters=letters) # POM (A), Algae (AB), Zoop (B), Plants (B)


###### Valine: 
Val.mod<-lm(Val~Group, data=ESS.df.raw)
print(anova(Val.mod), digits=6) # p=0.329
...
#### Permanova: Site and Group effects 
Use Permanova to test for differences among sites in measured and normalized data. Explore the differences at the base of the food web in δ^13^C values across sites.   
  
*Site effects*
- Use raw and normalized daga
- Remove the source, since collection unbalanced
- Run PERMANOVA to test for site effects. 
- This is part of Table S3.

This PERMANOVA (`EAA.raw.perm.Lake.zoop`) uses measured (non-normalized) data to test site effects on zooplankton.
```{r rawEAA perman Lake}
# master df = EAA.df.wide
# non-normalized df = ESS.df.raw

# remove the sources (POM and Plants), just look at the zooplankton which have site-level replications
PCA.raw.zoop<-ESS.df.raw[!(ESS.df.raw$phy.group=="Plants" | 
                        ESS.df.raw$phy.group=="POM" | 
                        ESS.df.raw$phy.group=="Algae"),]
...
This PERMANOVA (`EAA.norm.perm.Lake.zoop`) uses mean-normalized data to test site effects on zooplankton.
```{r normEAA perman Lake}
# master df = EAA.df.wide
# normalized df = ESS.df.norm

# remove the sources (POM and Plants), just look at the zooplankton which have site-level replications
PCA.norm.zoop<-ESS.df.norm[!(ESS.df.norm$phy.group=="Plants" | 
                        ESS.df.norm$phy.group=="POM" | 
                        ESS.df.norm$phy.group=="Algae"),]
...
*Group effects*  
- Use  measured and normalized data
...
This PERMANOVA `EAA.raw.perm.Group` uses raw/measured data, with 3 sources and zooplankton (pooled). 
...
# run PERMANOVA for 4 Groups, using measured data
set.seed(51)
EAA.raw.perm.Group<-adonis2(ESS.dat~ Group, data=Group.fac, permutations=999, 
                            method="euclidian", by="terms")
EAA.raw.perm.Group # different, Lake: p=0.004 R2=0.13, Group: p=0.001, R2=0.49
# with Group model, 54% R2

# run pairwise
set.seed(51)
pairwise.adonis(ESS.dat, factors=c(Group.fac$Group), sim.method = "euclidian", p.adjust.m = "none")
# All groups differ from each other, except POM and zoop.
```

This PERMANOVA `EAA.norm.perm.Group` uses normalized data, with 3 sources and zooplankton (pooled). 
...
# run PERMANOVA for 4 Groups
set.seed(51)
EAA.norm.perm.Group<-adonis2(ESS.n.dat~ Group, data=Group.n.fac, permutations=999, 
                        method="euclidian", by="terms")
EAA.norm.perm.Group # Group: p=0.001, R2=0.54
...
################################
## what if just sources? (spoiler: same outcome)
# run PERMANOVA for 3 Source groups
...
Source.n.fac<-ESS.norm.perm.source  %>%  dplyr::select(Lake, Group)
...
#### PCA 
Use the mean-normalized data to examine patterns in PCA space across sources (POM, algae, terrestrial plants)
*This is Figure S5*
