...
pacman::p_load('knitr', 'lme4', 'lmerTest', 'tidyverse', 'effects', 'plyr', 'dplyr', 'plotrix', 'car',"gridExtra", "cowplot", "tools", "mgcv", "gratia", "MASS", "stats", "tidymv", "sjstats", "coin", "emmeans", "ggplot2", "mda", "nortest", "reshape2", "gmm", "propagate","ggmap", "RgoogleMaps")
...
write.csv(lda.info, "output/SNL_Producers_LDA_loadings_NMX.csv")
...
write.csv(AlldataPredict, "output/SNL_Producers_LDA_coords_NMX.csv")
...
write.csv(zoop.predict.data, "output/SNL_Zooplankton_LDA_info_NMX.csv")
...
# remove TB tanks
LDA.df<-LDA.df[!(LDA.df$Lake=="TB-tanks"),]
...
# reorder factors 
LDA.df.el$orig.class<-factor(LDA.df.el$orig.class, levels=c(">350um", "calanoid", 
                                                            "copepods", "Ceriodaphnia", "Daphnia",
                                                            "Holopedium", "POM", "Plants"))
...
###
LDA.spp.lake<-ggplot(LDA.df.el, aes(x=LD1, y=Lake, 
                                 fill = orig.class, shape= orig.class))+
  geom_point(size=3)+
  geom_vline(xintercept=-0.75, linetype="dashed", color = "gray60")+
  annotate(geom="text", label="POM-source", x=-3.5, y=9.5, color="gray30", size=3) +
  annotate(geom="text", label="Plant-source", x=2.5, y=9.5, color="gray30", size=3) +
...
LDA.df.el$phy.group<-factor(LDA.df.el$phy.group, levels=c("POM", ">350um", "cladocera", 
                                                            "copepoda",
                                                            "Plants"))

LDA.phy.boxplot<-ggplot(LDA.df.el, aes(x=LD1, y=phy.group, 
                                 fill = phy.group))+
...
# reorder factors 
LDA.df.el$orig.class<-factor(LDA.df.el$orig.class, levels=c("POM", "Plants", ">350um", "calanoid", 
                                                            "copepods", "Ceriodaphnia", "Daphnia",
                                                            "Holopedium"))


density_LDA<-ggplot(data=LDA.df.el, aes(x=LD1, group=phy.group, fill=phy.group)) +
...
density.ridge<-ggplot(LDA.df.el, aes(x = LD1, y = phy.group, fill = after_stat(x))) +
...
#Load Packages for MixSIAR Models
```{r}
library(MixSIAR)
library(dplyr)
library(GGally)
...
colnames(source.agg.df) <- c("Type", "n", "MeanLD1", "SDLD1")

#Write and export a csv of the source data frame
write.csv(source.agg.df, "output/source.agg.df.csv")

```

#### Import Data and Create Model Structure
```{r}
###GO INTO THE ZOOPLANKTON FILE (Mix.df.csv) AND DELETE THE FIRST COLUMN###

###GO INTO THE SOURCE FILE (source.agg.df.csv) AND DELETE THE FIRST COLUMN###
...
library(plyr)
mod3.mixSIAR.output<-separate(mod3.mixSIAR.output, full.data, into = c("factor", "Mean", "SD", "2.5.perc", "5.perc", "25.perc", "50.perc", "75.perc", "95.perc"), sep=" Plants")
