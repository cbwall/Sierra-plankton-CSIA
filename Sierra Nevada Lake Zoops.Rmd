...
MSiar.data.env<- MSiar.data.env %>%
  dplyr::select("Location", "Date", "Lake", "elevation..m", "latitude", "longitude", "depth..m",
                "secchi..m", "temp..C", "DO..perc", "spc", "pH", "DOC..mg.L",
...
mod6<-lm(Prop.plant~TN..ug.L, data=Plant.nut.env); anova(mod6) # no relationship
mod7<-lm(Prop.plant~TP..ug.L, data=Plant.nut.env); anova(mod7) # no relationship
...
mod5<-lm(Prop.pom~pH, data=POM.nut.env); anova(mod5) # no relationship, but best one at p=0.2
mod6<-lm(Prop.pom~TN..ug.L, data=POM.nut.env); anova(mod6) # no relationship
mod7<-lm(Prop.pom~TP..ug.L, data=POM.nut.env); anova(mod7) # no relationship
mod8<-lm(Prop.pom~chla..ug.L, data=POM.nut.env); anova(mod8) # no relationship, R2=0.03
...
mod6<-lm(Prop.plant~TN..ug.L, data=copep.plant); anova(mod6) # no relationship
mod7<-lm(Prop.plant~TP..ug.L, data=copep.plant); anova(mod7) # no relationship
mod8<-lm(Prop.plant~chla..ug.L, data=copep.plant); anova(mod8) # no relationship, but best one, R2 0.19
...
### export it
pdf(file= "figures/Fig x.Plant.prop.mod.pdf", height=18, width=8)
prop.plant.mod.fig
dev.off()
...
plot_grid(elev.prop.plant, clad.elev.plant, copepod.elev.plant,
          elev.prop.plant, clad.chla, copepod.chla, ncol=3, nrow=2)
