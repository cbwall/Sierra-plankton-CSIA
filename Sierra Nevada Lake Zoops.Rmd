...
This PCA (`PC.norm.sorc`) uses mean-normalized data to examine patterns across sources (POM, algae, terrestrial plants). *This is Figure S5*
...
PC.norm.summ.sorc<-summary(PC.norm.sorc) #64% in 3 PCs
...
## PC1 and PC2
PCA.norm.sorc.plot <- ggbiplot(PC.norm.sorc, choices = 1:2, obs.scale = 1, var.scale = 1, 
  ...
  scale_shape_manual(values=c(24,24,21))+
  ggtitle("ESS-normalized: Sources")+
...
# export it
PCA.norm.sorc.plot
dev.copy(pdf, "figures/FigS5.PCA.Sources.pdf", height = 5, width = 6.5)
dev.off()
```
...
Run the LDA as biplot at different levels
```{r LD biplot}
...
LDA.scat.phygroup<-ggplot(LDA.df, aes(x=LD1, y=LD2))+
  ...
  scale_color_manual(values=lda.6.colors)+
  scale_fill_manual(values=lda.6.colors)+
  scale_shape_manual(values=c(24, 24, 21 ,21, 21, 24))+
  scale_linetype_manual(values=c(2,2,0,0,0, 2))+
  xlab("LD1") + ylab("LD2") +
  theme_classic() + guides(lty = "none")

LDA.compiled<-plot_grid(Source.LDA, LDA.scat.Group, LDA.scat.phygroup, ncol=3, labels=c("A", "B", "C"), rel_widths = c(1,1.2,1.2))

dev.copy(pdf, "figures/Fig2.Source.Plank.scat.LDA3.pdf", width = 14, height = 4)
dev.off()
```
...
Export zooplankton and source data to feed into MixSIAR. We will calculate the mean, standard deviations, and sample sized for LD1 values of the sources.
```{r mixsiar sources}
#Create a new data frame with desired columns (Zooplankton)
Mix.df.all <- LDA.df %>%
...
Mix.df <- Mix.df.all[!(Mix.df.all$orig.class=="Plants" | Mix.df.all$orig.class=="POM" |  Mix.df.all$orig.class=="Algae"),]
