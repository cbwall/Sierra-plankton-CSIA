...
PCA.dat.abs<-abs(PCA.dat)

# run PERMANOVA for Lake, NO SOURCE
set.seed(213)
EAA.perm.LAKE<-adonis2(PCA.dat.abs~Lake, data=PCA.fac, permutations=999, method="bray")
EAA.perm.LAKE # different at p=0.001

pairwise.adonis(PCA.dat.abs, factors=c(PCA.fac$Lake), p.adjust.m = "hochberg")
# below significant at 0.020
# Granite 2, Lukens, May Pond all pretty unique

# EasternBrook vs Granite2
# Cascade vs Granite2
# Blue vs Granite2
# MayPond vs Granite2

# MayPond vs EasternBrook
# MayPond vs EasternBrook
# MayPond vs Cascade
# MayPond vs Blue
...
# make the PCA df, remove proportion plant, keeping sources
PCA.df.normAA<-PCA.df %>% 
  dplyr::select(-Prop.plant)
...
#plot(PC.plank.norm, type="lines", main="PC.area eigenvalues")
...
# test hypotheses that these groups differ
...
# make absolute values for permanova
Perm.df.AA.abs<-abs(Perm.df.AAs)

# from above, rename the factors
Perm.fac<-Perm.df  %>% 
  dplyr::select(Lake, phy.group)

# run PERMANOVA for phy.group
set.seed(513)
EAA.perm<-adonis2(Perm.df.AA.abs~phy.group, data=Perm.fac, permutations=999, method="bray")
EAA.perm # different at p=0.026

pairwise.adonis(Perm.df.AA.abs, factors=c(Perm.fac$phy.group), p.adjust.m = "none")
# POM vs Plants: p=0.007
# POM vs cladocera: p=0.056
# Plants vs copepod: p=0.008
# cladocera vs. copepod: p=0.006
