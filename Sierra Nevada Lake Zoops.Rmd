...
mod.mult<-lm(Prop.plant~TN..ug.L+chla..ug.L,
             data=cladoc.plant)
summary(mod.mult)
sigma(mod.mult)/mean(Plant.nut.env$Prop.plant) #63% error rate
...
multregres.plots
dev.copy(pdf, "figures/Fig5.Multregrss.pdf", height=3.5, width=9)
dev.off()
```

### PCA-EAA
Make a raw-value PCA with values to explore the differnces at the base of the food web in d13C values.
```{r PCA lake rawEAA}
#PCA df with NAs removes
...
### export it
pdf(file= "figures/Fig6A.PCA.lake.pdf", height=8, width=8)
PCA.lake.plank
dev.off()
```

Make a PCA for plankton, mean-normalizing EAAs
```{r PCA zoops normEAA}
...
### export it
pdf(file= "figures/Fig6.PCA.norm.zoop.pdf", height=7, width=12)
