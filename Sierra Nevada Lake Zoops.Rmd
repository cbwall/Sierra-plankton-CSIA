...
dev.copy(pdf, "figures/FigS2.Sierra Nevada Lakes Ess AA Boxplot.pdf", width = 6.5, height = 6, encod="MacRoman")
dev.off()
...
#### Plotting Ess-AA d13C Fingerprints
- Raw data to see baseline differences across sites - ZOOPLANKTON ONLY
```{r}
...
# plot
ggplot(raw.EAA.reshape.grouped, aes(x = EAA, y = Mean, fill = Lake)) +
  theme_classic() +
  geom_point(size = 5, aes(color = Lake)) +
  geom_errorbar(aes(ymin = Mean-SE, ymax = Mean+SE, color = Lake), width = 0.2) +
...
# save plot
ggsave("figures/AA_d13C_Fingerprint_Lines.pdf", plot = last_plot())
...
### export it
pdf(file= "figures/Fig2A.PCA.norm.plank.pdf", height=7, width=6)
...
LDA.phy.boxplot
dev.copy(pdf, "figures/Fig2B.LDA.phy.boxplot.pdf", height = 6, width = 6)
dev.off()

## Groups the PCA and LDA into a single figure
### export it
pdf(file= "figures/Fig2AB.PCA.LDA.comb.pdf", height=5, width=10)
plot_grid(PCA.norm.plank, 
          LDA.phy.boxplot+theme(legend.position="none"))
dev.off()
