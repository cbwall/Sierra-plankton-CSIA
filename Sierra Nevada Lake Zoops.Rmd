...
### Multivariate stats
We will do a series of PERMANOVA and PCAs. 

(1) Using the non-normalized data, test for differences among all the data at two levels using a PERMANOVA
```{r permanova prep}
...
set.seed(51)
EAA.perm.GROUP<-adonis2(ESS.groups.dat.abs~ Group, data=Group.fac, permutations=999, method="bray")
EAA.perm.GROUP # not-different, p=0.064

pairwise.adonis(ESS.groups.dat.abs, factors=c(Group.fac$Group), p.adjust.m = "none")
# Plants differ from POM (p=0.003), plants almost significant from Zooplankton (p=0.091)

####################
#### drop the zooplankton and just look at the sources
ESS.sources<- ESS.mods.df %>% dplyr::select(-c(orig.class, phy.group))
ESS.sources<- ESS.sources[(ESS.sources$Lake=="Source"),]

# just AAs data
ESS.sources.dat<-ESS.sources  %>% 
  dplyr::select(Ile, Leu, Phe, Thr, Val)

# make absolute values for permanova (requires + values)
ESS.sources.dat.abs<-abs(ESS.sources.dat)

# factors
Source.fac<-ESS.sources  %>% 
  dplyr::select(Group)

# run PERMANOVA for 2 groups: JUST SOURCES
set.seed(500)
EAA.perm.SOURCE<-adonis2(ESS.sources.dat.abs~ Group, data=Source.fac, permutations=999, method="bray")
EAA.perm.SOURCE # not-different, p=0.0004


```

- compare sources to plankton phy groups (5 levels) 
```{r permanova 2}
#### #### #### #### #### #### #### #### #### #### #### 
...
set.seed(13)
EAA.perm.PHY<-adonis2(ESS.phy.dat.abs~ phy.group, data=Phy.fac, permutations=999, method="bray")
EAA.perm.PHY # p = 0.049

pairwise.adonis(ESS.phy.dat.abs, factors=c(Phy.fac$phy.group), p.adjust.m = "none")
# plants differ from POM (p=0.002)
# plants vs copepods (p=0.013)
...
set.seed(213)
EAA.perm.LAKE<-adonis2(PCA.dat.raw.abs~Lake, data=PCA.fac, permutations=999, method="bray")
...
- this compared the sources relative to zooplankton phy.groups
```{r PCA zoops normEAA}
...
# run PERMANOVA
set.seed(213)
EAA.perm.norm.plank<-adonis2(PCA.dat.norm.abs~ phy.group, data=PCA.fac.norm, permutations=999, method="bray")
