...
d13C.d15N<-ggplot(bulk.iso, aes(x=d13C, y=d15N, color=Type)) +
  scale_color_manual(values=c("darkslategray4", "coral", "royalblue1", "wheat3", "wheat4")) +
  ...
  stat_ellipse()+
  theme_classic()
ggsave("figures/FigS1.Bulk_13C.15N.pdf", height=5, width=6)
...
Make point-line plots (mean +/-SE) for the individual AA~ESS~, both raw/measured and mean normalized. Do this for each producer group (algae, POM, terrestrial plants) and zooplankton. This will allow us to evaluate effects of normalization on different groups. *This is Figure S3A-B*.
...
###############
pdf(file= "figures/FigS3.indivAA_Lines_rev.pdf", height=9, width=12)
plot_grid(indiv.EAA.plot, indiv.EAA.plot.norm, rel_widths = c(1.65,2.1), labels=c("A", "B"))
dev.off()
```

##### Boxplots: raw & norm
We see clear effects of sampling location and sample types that reflect elevation of Lakes (possibly in interaction with other environmental factors) as well as biology, notably in benthic algae.  *This is Figure S4A-B*.

```{r EssAA boxplot, fig.dim=c(5,4), results='hide', fig.cap="**Figure S4**. Boxplot of *(A)* measured and *(B)* mean-normalized essential amino acids (AA~EES~) δ^13^C values for benthic algae, particulate organic matter (POM), terrestrial plants, and zooplankton in aquatic habitats of the Eastern Sierra Nevada mountains."}
...
dev.copy(pdf, "figures/FigS4.SNL.EssAA.Boxplot_rev.pdf", width = 10, height = 5)
dev.off()
```
...
# get the df organized for the analyses.
##### measured data, non-norm #####
...
#### PCA 
Use the mean-normalized data to examine patterns in PCA space across the zooplankton groups relative to their sources (POM, algae, terrestrial plants)
*This is Figure S5*
```{r PCA zoops, results='hide', fig.dim=c(6,5), fig.cap="**Figure S5**. Principal component analysis (PCA) for mean-normalized AA~ESS~ δ^13^C values (AA~ESS-norm~) for zooplankton Groups and sources (POM, terrestrial plants). Ellipses represent 90% confidence intervals, where Group sample sizes are n ≥ 4."}
...
# export it
PCA.norm.plot
dev.copy(pdf, "figures/FigS5.PCA.pdf", height = 5, width = 8)
dev.off()
```
...
# export it
PCA.norm.sorc.plot
dev.copy(pdf, "figures/Figx.PCA.Sources.pdf", height = 5, width = 6.5)
dev.off()
```
